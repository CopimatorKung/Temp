...
ก่อนเข้า platform ผู้ใช้ต้องผ่านหน้า `/login` ด้วย email/password และ backend ต้องมี `POST /auth/login` กับ `GET /auth/me` เพื่อคืน role, permission, sales profile และ badge ที่ใช้แสดงใน navbar/sidebar
...
- track สามารถมี prerequisite track ได้ เช่น `Chatbot Business` ต้องผ่าน `Chatbot Basic I` และ `Chatbot Basic II` ก่อน จึงจะเริ่มเรียนหรือ assign ให้ sales ได้
...
- sales เห็น prerequisite ของ track และสถานะ locked/unlocked ชัดเจน ถ้ายังไม่ผ่าน track ที่จำเป็น
...
- manager/admin เข้า `track-management/:id` เพื่อแก้ track, prerequisite tracks, topic order, source, required score และ badge threshold ได้
