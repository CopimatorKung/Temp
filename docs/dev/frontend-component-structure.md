...
Theme ปัจจุบันคือ **Morphous Lily of the Valley** โดยมี reference assets:
...
- ใช้โทน bell white, petal ivory, pale leaf vein, fresh leaf green, shaded stem green, moss-gray border และ cool shade ink ตาม theme reference
- ห้ามสร้าง palette ใหม่ที่ไม่อิง `ui_color_theme.json`
- Tailwind tokens และ shadcn/ui CSS variables ต้อง derive จาก theme นี้
- primary action ควรใช้ shaded stem green
- success/focus state ใช้ fresh leaf green หรือ pale vein focus ring
- surface ใช้ bell white/petal ivory ใน light mode และ cool shade ink/deep leaf shadow ใน dark mode
- border, shadow และ radius ต้องคงแนว premium product system, radius หลัก 8px
- ถ้าต้องแปลงเป็น token จริง ให้สร้างไฟล์ token แยกจาก reference นี้ เช่น `src/styles/theme.css` หรือ `tailwind.config.ts`
