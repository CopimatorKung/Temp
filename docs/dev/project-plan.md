...
### 5.3 Playbook / Knowledge Structure

Playbook ควรเป็น Knowledge Library ที่มีโครงสร้างเหมือนหนังสือ ไม่ใช่ raw document dump โดย UI จัดเป็น `book -> chapter -> topic -> page` และให้ `page` เป็นหน่วยหลักสำหรับ editing, citation, governance และ RAG indexing

| Field | รายละเอียด |
|---|---|
| playbook_title | ชื่อ playbook เช่น Product A Sales Playbook, Q2 Promotion Playbook |
| chapter_title | หมวดใหญ่ใน book เช่น Promotion Governance, Competitor Handling |
| topic_title | หัวข้อย่อย เช่น Q2 Promotion Terms, Safe Comparison |
| page_title | หน้า content ที่ user อ่าน แก้ไข favorite และใช้เป็น citation |
| section_type | FAQ, Product, Pricing, Promotion, Competitor, Objection, Compliance, Talk Track |
| markdown_body | เนื้อหาหลักที่เขียนด้วย Markdown พร้อม preview ในหน้า Knowledge |
...
Resource import ต้องรองรับ `.pdf`, `.csv`, `.xlsx`, `.md`, `.doc`, `.docx`, `.txt` โดย backend แปลงเป็น normalized text artifact และให้ owner map เข้า book/chapter/topic/page ก่อน publish หรือ sync เข้า RAG index
...
1. Owner สร้างหรืออัปเดต Knowledge Book, Chapter, Topic หรือ Page
2. Owner เขียนเนื้อหาใน Markdown editor หรือ upload resource เช่น PDF/CSV/XLSX/MD/DOC/DOCX/TXT แล้ว map เป็น page
3. Product, training หรือ legal review
4. Approve page
5. สร้าง/อัปเดต full-text index จาก hierarchy title, markdown body, extracted text และ tags
6. ถ้าเปิด local RAG ให้ sync approved page ไปยัง Kotaemon/LEANN index โดยเก็บ mapping กลับมาที่ `knowledge_page_id`
7. ตรวจ effective_date/expiry_date โดยเฉพาะ promotion และ pricing
8. Publish ไปยัง production
9. Run evaluation ด้วยชุดคำถาม gold set
10. เก็บ feedback จากคำถามที่ตอบไม่ได้หรือ citation ไม่ดี
11. เก็บ favorite/bookmark จาก Senario หรือ session review เพื่อให้ user กลับมาอ่านในหน้า Knowledge
12. Auto-hide หรือ mark expired เมื่อพ้น expiry_date
...
1. Admin publish Knowledge Page ใน SaleSync
2. Backend สร้าง normalized text artifact และ metadata เช่น `knowledge_page_id`, book/chapter/topic, product, tags, effective/expiry, status
...
7. Backend compose answer พร้อม citation ที่ map กลับมาที่ Knowledge Page ไม่ใช่ raw chunk ลอย ๆ
...
| Track Topic Type | ตัวอย่างเนื้อหา | วิธีวัดผล |
|---|---|---|
| Knowledge | positioning, target market, brand promise | read/acknowledge หรือ short quiz |
| External View | demo, reference doc, customer story | open/view completion |
| Audio Response | AI เล่นเสียงโจทย์ให้ sales เขียน response | AI/manager score |
| Recording Review | browser record หรือ upload audio attempt | training rubric score |
| Senario / Meeting Room | persona หรือ multi persona simulation | session score ผ่าน threshold |
| Manager Sign-off | evidence review ก่อนรับ lead จริง | manager approval |
...
- สร้าง onboarding track และ badge threshold ได้
- assign track ให้ sales ได้
- sales ทำ knowledge topic, external view, audio response, Senario และ pitch practice ได้
- manager เห็น progress และให้ sign-off ได้
- Senario ที่ผูกกับ track สามารถ complete topic และ unlock badge เมื่อผ่าน percent threshold ได้
...
| Knowledge Editor | Tiptap + `@tiptap/markdown` | เขียน Knowledge Page แบบ rich editor แต่ serialize กลับเป็น Markdown source พร้อม preview |
...
| OnboardingTrack | track, topic, source ref และ badge requirement ที่ sales ต้องผ่าน |
| OnboardingTopicProgress | สถานะของ sales ต่อแต่ละ topic ใน track |
| UserBadge | badge ที่ sales ได้รับจาก onboarding track |
...
- Track Management สำหรับสร้าง/แก้ topic, source ref, Senario link และ badge threshold
- Track detail และ progress ราย topic
- Badge unlock
- Audio response และ training task
- Manager sign-off
- Dashboard รวม track progress, badge, pitch score และ call quality signal
...
| Onboarding | time to readiness, track completion rate, topic completion rate, badge unlock rate |
...
- Onboarding track builder
- Track assignment
- Topic management
- Badge management
- Progress tracking
- Audio response topic
- Linked Senario completion
- Manager sign-off
- Readiness dashboard
...
- Recording review mode
- Basic onboarding track, topic progress and badge unlock
