...
const navItems = [
  {
    ...
    icon: FiBarChart2,
    activePrefixes: ["/app/dashboard"],
  },
  {
    label: "Quality",
    href: routes.audioNew,
    icon: FiCheckSquare,
    activePrefixes: ["/app/audio", "/app/template"],
  },
  {
    label: "Ask",
    href: routes.trainingAsk,
    icon: FiSearch,
    activePrefixes: ["/app/training/ask"],
  },
  {
    label: "Recordings",
    href: routes.recordingReview,
    icon: FiUploadCloud,
    activePrefixes: ["/app/training/recording-review", "/app/training/rubrics"],
  },
  {
    label: "Senario",
    href: routes.voiceRoleplay,
    icon: FiMic,
    activePrefixes: ["/app/training/voice-roleplay"],
  },
  {
    label: "Knowledge",
    href: routes.playbooks,
    icon: FiBookOpen,
    activePrefixes: ["/app/playbooks", "/app/knowledge"],
  },
  {
    label: "Onboard",
    href: routes.onboardingMe,
    icon: FiFileText,
    activePrefixes: ["/app/onboarding"],
  },
  {
    label: "Settings",
    href: routes.settings,
    icon: FiSettings,
    activePrefixes: ["/app/settings", "/app/admin"],
  },
];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className={[
        "min-h-screen overflow-x-hidden lg:grid lg:h-screen lg:overflow-hidden",
        isCollapsed
    ...
    >
      <aside className="sticky top-0 z-40 flex max-h-screen min-w-0 border-b border-sidebar-border bg-sidebar text-sidebar-foreground transition-[width] lg:h-screen lg:flex-col lg:border-b-0 lg:border-r">
        <div
          className={[
            "flex h-14 items-center border-sidebar-border px-3 lg:border-b",
            isCollapsed ? "justify-center gap-1.5 lg:px-2" : "justify-between gap-2",
          ].join(" ")}
        >
          <Link
            to={routes.dashboard}
            ...
            className={[
              ...
              isCollapsed
                ? "hidden h-8 w-8 place-items-center bg-sidebar-accent text-[13px] font-semibold text-sidebar-primary shadow-sm ring-1 ring-sidebar-primary/12 lg:grid"
                : "block",
            ].join(" ")}
          >
            {isCollapsed ? (
          ...
          <button
            ...
            className={[
              "hidden shrink-0 items-center justify-center rounded-lg text-sidebar-foreground/70 transition hover:bg-sidebar-accent hover:text-sidebar-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring lg:flex",
        ...
        <nav className="flex flex-1 gap-1 overflow-auto p-2.5 lg:grid lg:auto-rows-min">
          {navItems.map((item) => {
            const isRouteActive = item.activePrefixes.some((prefix) =>
      ...
      <main className="flex min-w-0 flex-col lg:h-screen lg:overflow-y-auto">
        <Breadcrumbs />
        {children}
      </main>
    </div>
  ...
  return (
    <div className="relative border-sidebar-border p-2.5 lg:border-t">
      ...
      >
        ...
        <div
          className={["min-w-0 flex-1", isCollapsed ? "hidden" : ""].join(" ")}
        >
      ...
      {open && (
        <div
          ...
          className={[
            ...
            isCollapsed
              ? "left-3 lg:left-[calc(100%+8px)] lg:right-auto lg:bottom-3"
              : "left-3 right-3",
          ].join(" ")}
        >
          <Link
          ...
          <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/35 p-4 backdrop-blur-sm">
            ...
            >
              <div className="border-b border-border p-5">
                ...
                >
                  {logoutConfirmed
                    ? "Logged out from mock session"
                    : "Logout from Pitchsmith?"}
                </h2>
